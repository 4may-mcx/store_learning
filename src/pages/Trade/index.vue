  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div
        ...
        v-for="address in addreddInfo"
        :key="address.id"
      >
        <span class="username" :class="{ selected: address.isDefault == 1 }">{{
          address.consignee
        }}</span>
        <p @click="changeDefault(address, addreddInfo)">
...
export default {
  ...
  computed: {
    ...mapState({
      addreddInfo: (state) => state.trade.address,
      orderInfo: (state) => state.trade.orderInfo,
    }),
    userDefaultAddress() {
      return this.addreddInfo.find((item) => item.isDefault == 1);
    },
  },
  methods: {
    // 修改默认地址
